...
We'll improve our thresholding algorithms from the [previous article](/p/2023-07-webgl-07-threshold) by replacing our block colors with patterns that, from a distance, look like a shade of a color. Here's an example of rendering a horizontal gradient with different dithering effects we'll cover in this article.
...
We will check each pixel of our image against this matrix, repeating for the size of the image. For the matrix above the first pixel on the top row will be thresholded to the value 0.2. Anything above 0.2 turning white and anything below turning black. The second pixel by 0.8 and the third pixel will loop back to the first value in our matrix 0.2.
...
The data for our patterns is normally stored as integers and we use this equation to create our threshold values.
...
The beauty of this algorithm is we can easily expand on it with different patterns and matrix sizes for a number of different results.
