...
We can simulate this effect by linearly moving one or more of our RGB channels from our texture along the x or y plane. Here are two examples of moving the red and blue channels:

<Dither step="chromatic-aberration-red" />

<Dither step="chromatic-aberration-blue" />

The GLSL code for this is quite straightforward. Below we pass our texel data into our chromatic aberration function. This generates a vec4 color value based on the texel at our program's uv. We can then move our red channel by an `xOffset` and `yOffset`. 
...
Instead of linearly moving each color channel we can increase the amount by the distance from the center, this mimics the effect seen in lenses. Similar to our vignette effect in the [previous article](/p/2023-07-webgl-09-vignette).
