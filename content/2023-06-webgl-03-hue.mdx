...
Hue is the main indication of a color. It's the value that defines the color itself. It is the "red" of a tomato and the "yellow" of a banana.

Changing the hue of a photo will change the color of an object, meaning that a tomato would no longer be red, or that a banana would no longer be yellow.
...
Here are two great diagrams from [Wikipedia](https://en.wikipedia.org/wiki/HSL_and_HSV) to illustrate the difference between RGB on the left, and HSV (or HSB) on the right. 
...
But the green range manages to change all the green leaves in the background.

In GLSL we can achieve this by only updating the hue if the texel's hue value lies in between the range min and max of our target as the function below shows.
...
In the next article, we will look into adding an analog feel to our images by [simulating Film Grain.](/p/2023-06-webgl-04-film-grain)
