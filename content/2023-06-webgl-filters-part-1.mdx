---
title: "WebGL photos - Color correction"
...
WebGL enables us to **send data to** and **run programs (shaders)** on the GPU. This series will focus on a type of shader program know as a **fragment shader**. A fragment shader will **run once for each pixel on the rendered image**. These shader programs run in parallel so you can't have any shared state between them.
...
We pass in data to our fragment shader as a **Uniform**. A Uniform is a variable that is the same for every fragment shader in a render pass. As this series will focus on editing an image, we'll pass this into our shader program as a special kind of Uniform know as a **Texture**. Textures are WebGL's built in support for images. We'll store the image data as a 2x2 matrix (row x colums) of vec4 color (rgba) values for each pixel cell.
...
This program doesn't do much at the moment. It simply outputs the image we pass in, as we can see below.
