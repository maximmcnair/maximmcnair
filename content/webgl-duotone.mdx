...
Duotone is a simple effect that maps a black-and-white image into a color range. This effect dates back to the 50/60s with Andy Warhol using variations of it. Its still in use today, Spotify makes extensive use of it across its advert campaigns.
...
First we'll need a black and white image. We don't want to be limited to just using black and white images. So let's reuse our desaturated function we covered in the [color correction article](/p/webgl-color-correction) of this series to create one.
