...
We’ll start off with a basic introduction to WebGL and GLSL, and then move onto simple color correction algorithms that you’d find in software such as Lightroom, VSCO or Instagram. We will then take a tour through numerous of filters and effects that would be at home in Photoshop.
...
WebGL is a JavaScript API for rendering high-performance, interactive, 3D and 2D graphics in the browser. It uses your **GPU** to compute your programs in parallel rather than the linear fashion of the CPU.
...
Shader programs are written in a C-inspired language called GLSL (OpenGL Shading Language). Graphics cards are extremely fast in comparssion to the CPU, and have many mathematical functions built in that GLSL enables us to take advantage of from our shader programs.
